...
func TestRundeck(t *testing.T) {
	...
	keyPair := aws.CreateAndImportEC2KeyPairContext(t, ctx, awsRegion, keyPairName)
	defer aws.DeleteEC2KeyPairContext(t, ctx, keyPair)
	...
	serverURL := terraform.OutputContext(t, ctx, terraformOptions, "server_url")
	...
	retry.DoWithRetryContext(t, ctx, "Wait for SSH", 20, 15*time.Second, func() (string, error) {
		return ssh.CheckSSHCommandContextE(t, ctx, &host, "echo ready")
	})
	...
	t.Run("rundeck_service_running", func(t *testing.T) {
		result, err := ssh.CheckSSHCommandContextE(t, ctx, &host, "sudo systemctl is-active rundeckd")
		require.NoError(t, err)
		assert.Contains(t, result, "active")
	})

	t.Run("nginx_service_running", func(t *testing.T) {
		result, err := ssh.CheckSSHCommandContextE(t, ctx, &host, "sudo systemctl is-active nginx")
		require.NoError(t, err)
		assert.Contains(t, result, "active")
	})

	t.Run("postgresql_service_running", func(t *testing.T) {
		result, err := ssh.CheckSSHCommandContextE(t, ctx, &host, "sudo systemctl is-active postgresql-16")
		require.NoError(t, err)
		assert.Contains(t, result, "active")
	})
	...
	t.Run("postgresql_accepting_connections", func(t *testing.T) {
		result, err := ssh.CheckSSHCommandContextE(t, ctx, &host, "sudo -u postgres /usr/pgsql-16/bin/pg_isready -h localhost -p 5432")
		require.NoError(t, err)
	...
	t.Run("cloudwatch_log_group_configured", func(t *testing.T) {
		logGroupName := terraform.OutputContext(t, ctx, terraformOptions, "cloudwatch_log_group_name")
		require.NotEmpty(t, logGroupName)
		// Verify the CloudWatch agent config on the instance references the correct log group
		result, err := ssh.CheckSSHCommandContextE(t, ctx, &host,
